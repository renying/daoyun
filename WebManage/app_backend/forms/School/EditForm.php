...
namespace app_backend\forms\School;
        ...
        if($this->validate())
        {
            ...
            if($Menu->save())
            {
                if(!$Menu->enabled)
                {
                    ...
                    $child_ids && SchoolExtends::updateAll(['enabled' => $Menu->enabled], ['in', 'menuid', $child_ids]);
                }
                ...
                if($old_parent_id != $this->parent_id)
                {
                    ...
                    {
                        $OldParentCategory = SchoolExtends::find()->where(['menuid' => $old_parent_id])->one();
                        $old_parent_get_ids = BackendHelpers::getParentIdsAndChildIds('school_datas', 'menuid', $old_parent_id);
                    ...
                    {
                        $OldChildCategory = SchoolExtends::find()->where(['menuid' => $old_child_id])->one();
                        $old_child_get_ids = BackendHelpers::getParentIdsAndChildIds('school_datas', 'menuid', $old_child_id);
                    ...
                    $get_ids = BackendHelpers::getParentIdsAndChildIds('school_datas', 'menuid', $Menu->menuid);
                    ...
                    if($Menu->save())
                    {
                        ...
                        {
                            $ParentCategory = SchoolExtends::find()->where(['menuid' => $parent_id])->one();
                            $parent_get_ids = BackendHelpers::getParentIdsAndChildIds('school_datas', 'menuid', $parent_id);
                        ...
                        {
                            $ChildCategory = SchoolExtends::find()->where(['menuid' => $child_id])->one();
                            $child_get_ids = BackendHelpers::getParentIdsAndChildIds('school_datas', 'menuid', $child_id);
